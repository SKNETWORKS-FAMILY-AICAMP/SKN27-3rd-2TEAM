...
- `/api/chatbot/respond/stream` SSE 기반 스트리밍 응답 API
...
3. 백엔드는 KAG, RAG, Intent, Recommendation 단계로 추천 후보와 근거를 확정한다.
4. 백엔드는 자연어 응답 텍스트를 SSE `delta` 이벤트로 전송한다.
5. 프론트는 마지막 봇 말풍선을 `delta` 수신마다 갱신한다.
6. 백엔드는 응답 생성이 끝나면 최종 `response_state` 기준으로 Redis history/context/latest state를 1회 저장한다.
...
스트리밍 중간에는 DB나 Redis에 완료 상태를 저장하지 않는다. 최종 `response_state`가 만들어진 뒤 Redis 저장을 먼저 완료하고 `final` 이벤트를 전송한다.

LLM 스트리밍이 사용 불가한 환경에서는 로컬 응답 문자열을 토큰 단위로 나눠 SSE `delta`로 전송한다.
...
1. DB schema와 query constant 추가
2. Redis taste_events key/helper 추가
3. TasteEventService와 API 추가
4. SessionFlushService에 취향 이벤트/profile flush 추가
5. MusicDetailModal에 `내 취향에 추가` 액션 연결
6. 홈 이동 확인 모달과 flush/clear 흐름 연결
7. 스트리밍 챗봇 endpoint 추가
8. 프론트 스트리밍 수신 및 chat store 확장
9. 백엔드/프론트 테스트 추가
