...
현재 `/app/rag` 아래 폴더 기준으로는 모든 폴더를 동시에 구현하기보다, "입출력 계약을 먼저 고정하고 그 다음에 내부 실행 흐름을 채우는 순서"로 가는 것이 가장 안전합니다.
특히 지금 문서에 적힌 조건처럼 `KAG_STATE는 입력으로만 사용`, `RAG는 RAG_STATE만 반환`, `최종 자연어 응답은 다른 계층에서 생성`이 핵심 제약이므로, 
이 제약을 먼저 코드 구조에 박아두는 편이 이후 수정 비용이 가장 낮습니다.

### 1. `ragStateBuilder`부터 시작

가장 먼저 구현해야 하는 영역은 `ragStateBuilder`입니다.

- 이유 1: 이 폴더가 RAG의 최종 산출물인 `RAG_STATE` 구조를 결정합니다.
- 이유 2: `services`, `adapters`, `contractValidator`가 모두 결국 이 상태 객체를 기준으로 연결됩니다.
- 이유 3: 여기서 스키마와 노드 책임이 먼저 고정되어야 나머지 폴더가 흔들리지 않습니다.

우선순위는 아래처럼 잡는 것이 좋습니다.
...
처음부터 LangGraph 전체를 완성하려고 하기보다, 우선은 `RAG_STATE` 필드와 각 node 입출력 계약을 먼저 고정하는 MVP 구현이 적합합니다.

### 2. `contractValidator`를 바로 붙이기

두 번째 시작점은 `contractValidator`입니다.

- 이유 1: `RAG_STATE`가 잘 만들어졌는지 가장 빨리 검증할 수 있습니다.
- 이유 2: Mock 기반으로 흐름을 붙여도 결과가 계약에 맞는지 자동 확인할 수 있습니다.
- 이유 3: 이후 Real RAG로 확장할 때 회귀 테스트 기준점 역할을 합니다.

권장 순서는 아래와 같습니다.

1. `base_validator.py`
2. `format_validator.py`
3. `logic_validator.py`
4. `hallucination_validator.py`

MVP 단계에서는 `format_validator`와 `logic_validator` 위주로 먼저 구현하고, `hallucination_validator`는 실제 retrieval context가 붙는 시점에 강화하면 됩니다.

### 3. `musicCatalogRepository`로 데이터 접근 경계 고정

세 번째는 `musicCatalogRepository`입니다.

- 이유 1: RAG가 어떤 원천 데이터에서 근거를 가져오는지 경계를 먼저 정해야 합니다.
- 이유 2: 서비스 계층이 DB 세부 구현을 직접 알지 않도록 분리할 수 있습니다.
- 이유 3: Mock 데이터와 실제 SQL 구현을 같은 인터페이스 뒤에 숨길 수 있습니다.

권장 순서는 아래와 같습니다.

1. `base_repostiory.py`
2. `sql_repostiory.py`

여기서는 처음부터 Elasticsearch나 벡터 검색까지 넣기보다, "콘텐츠 ID 조회", "메타데이터 조건 조회", "추천 후보 목록 반환" 정도의 최소 조회 계약부터 잡는 것이 좋습니다.

### 4. `adapters`로 Mock/Real 교체 지점 만들기

네 번째는 `adapters`입니다.

- 이유 1: 서비스 계층이 Mock과 Real 구현을 같은 방식으로 호출할 수 있게 됩니다.
- 이유 2: MVP에서는 `rag_mock_adapter.py`만으로도 전체 흐름 검증이 가능합니다.
- 이유 3: 이후 `rag_real_adapter.py`를 붙여도 상위 계층 변경을 최소화할 수 있습니다.

구현 순서는 아래가 적절합니다.

1. `rag_adapter.py`
2. `rag_mock_adapter.py`
3. `rag_real_adapter.py`

초기에는 `rag_real_adapter.py`를 비워두거나 인터페이스 수준으로만 만들어도 괜찮습니다. 중요한 것은 `Service -> Adapter -> RAG_STATE 반환` 흐름을 먼저 안정화하는 것입니다.

### 5. `services`는 가장 마지막에 오케스트레이션

다섯 번째는 `services`입니다.

- 이유 1: 이 계층은 실제 비즈니스 흐름을 묶는 오케스트레이션 계층이라, 아래 계약이 먼저 정리되어야 구현이 깔끔합니다.
- 이유 2: 현재 구조상 `services`는 직접 복잡한 판단을 하기보다 builder, repository, adapter를 조합하는 책임이 더 큽니다.

MVP에서는 아래 우선순위를 추천합니다.
...
- `rag_generation.py`: 현재 구조의 메인 진입점 역할을 먼저 만들기 좋습니다.
- `retrieval.py`: MVP에서는 실제 벡터 검색이 아니라 repository + mock evidence 조합으로도 먼저 구현 가능합니다.
- `embedding.py`, `indexing.py`: Real RAG 확장 단계에서 본격 구현하는 편이 자연스럽습니다.

### MVP 기준 추천 구현 순서

아래 순서로 진행하면 중간에 막히지 않고 결과를 빨리 확인할 수 있습니다.

1. `ragStateBuilder/schema.py`에서 `RAG_STATE` 확정
2. `contractValidator/base_validator.py`, `format_validator.py` 구현
3. `musicCatalogRepository/base_repostiory.py` 인터페이스 정의
4. `adapters/rag_adapter.py` 인터페이스 정의
5. `adapters/rag_mock_adapter.py`로 Mock 기반 `RAG_STATE` 반환 구현
6. `ragStateBuilder/nodes.py`, `edges.py`, `builder.py`로 MVP 흐름 연결
7. `services/rag_generation.py`에서 상위 호출 진입점 구성
8. `contractValidator/logic_validator.py`까지 붙여 계약 검증 완료

이 순서의 장점은 "실제 검색 엔진이 없어도 먼저 동작하는 RAG 골격"을 만들 수 있다는 점입니다.

### Real RAG 확장 순서

MVP가 안정화된 뒤에는 아래 순서로 확장하는 것이 좋습니다.

1. `musicCatalogRepository/sql_repostiory.py` 실제 DB 조회 강화
2. `services/retrieval.py`에 실제 retrieval 로직 추가
3. `services/embedding.py`에 임베딩 생성 연결
4. `services/indexing.py`에 인덱싱 파이프라인 연결
5. `adapters/rag_real_adapter.py`에 LangGraph/외부 모델 연동
6. `contractValidator/hallucination_validator.py`에 grounding 검증 강화

즉, MVP는 "계약과 흐름", Real RAG는 "검색 품질과 외부 연동" 중심으로 나누는 것이 맞습니다.

### 폴더 기준 최종 착수 순서

폴더 단위로만 아주 간단히 정리하면 아래 순서가 가장 추천됩니다.

1. `ragStateBuilder`
2. `contractValidator`
3. `musicCatalogRepository`
4. `adapters`
5. `services`
